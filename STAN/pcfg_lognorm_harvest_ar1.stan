...
model {  // --------------------------------------------------------------------
  ...
  for(t in 2:n_dat_yrs){
    lambda[t] ~ normal(lambda[t - 1], sigma_lambda); // model assumes no autocorrelation in sigma
  }
...
generated quantities{  // ------------------------------------------------------
 ...
 logN_proj[1] = log(exp(normal_rng(logN[n_dat_yrs] * lambda_proj[1], sigma_lambda)) - N_harvest[1]);  
 
 for(t_proj in 2:n_proj_yrs){
   lambda_proj[t_proj] = normal_rng(lambda_proj[t_proj - 1], sigma_lambda);
   logN_proj[t_proj] = log(exp(normal_rng(logN_proj[t_proj - 1] * lambda_proj[t_proj], sigma_lambda)) - N_harvest[t_proj]);
 }
}
